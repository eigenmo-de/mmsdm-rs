        ...
        for file_key in self.data.keys() {
            match (
                    ...
                    #[cfg(feature = "mcc_dispatch")]
                    {
                        ...
                        for chunk in d.chunks(100_000_usize) {
                            ...
                            client
                                ...
                                .await?;
                            let num =
                                rust_decimal::Decimal::new(current.try_into().unwrap(), 0_u32);
                            let denom =
                                rust_decimal::Decimal::new(total.try_into().unwrap(), 0_u32);
                            dbg!(num / denom);
                        }
                    }
                ...
                ("FORECAST", "INTERMITTENT_GEN_DATA", 1_i32) => {
                    ...
                    {
                        let d: Vec<demand_forecasts::ForecastIntermittentGenData1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("OPERATIONAL_DEMAND", "ACTUAL", 1_i32) => {
                    #[cfg(feature = "demand_forecasts")]
                    {
                        let d: Vec<demand_forecasts::OperationalDemandActual1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("DEMAND", "MTPASA_INTERMITTENT_AVAIL", 1_i32) => {
                    #[cfg(feature = "demand_forecasts")]
                    {
                        let d: Vec<demand_forecasts::DemandMtpasaIntermittentAvail1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("DEMAND", "INTERMITTENT_GEN_LIMIT", 1_i32) => {
                    #[cfg(feature = "demand_forecasts")]
                    {
                        let d: Vec<demand_forecasts::DemandIntermittentGenLimit1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("DEMAND", "MTPASA_INTERMITTENT_LIMIT", 1_i32) => {
                    #[cfg(feature = "demand_forecasts")]
                    {
                        let d: Vec<demand_forecasts::DemandMtpasaIntermittentLimit1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("DEMAND", "INTERMITTENT_DS_RUN", 1_i32) => {
                    #[cfg(feature = "demand_forecasts")]
                    {
                        let d: Vec<demand_forecasts::DemandIntermittentDsRun1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("DEMAND", "INTERMITTENT_DS_PRED", 1_i32) => {
                    #[cfg(feature = "demand_forecasts")]
                    {
                        let d: Vec<demand_forecasts::DemandIntermittentDsPred1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("DEMAND", "INTERMITTENT_CLUSTER_AVAIL_DAY", 1_i32) => {
                    #[cfg(feature = "demand_forecasts")]
                    {
                        let d: Vec<demand_forecasts::DemandIntermittentClusterAvailDay1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("DEMAND", "INTERMITTENT_CLUSTER_AVAIL", 1_i32) => {
                    #[cfg(feature = "demand_forecasts")]
                    {
                        let d: Vec<demand_forecasts::DemandIntermittentClusterAvail1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("FORECAST", "INTERMITTENT_GEN", 1_i32) => {
                    #[cfg(feature = "demand_forecasts")]
                    {
                        let d: Vec<demand_forecasts::ForecastIntermittentGen1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("DEMAND", "INTERMITTENT_GEN_LIMIT_DAY", 1_i32) => {
                    #[cfg(feature = "demand_forecasts")]
                    {
                        let d: Vec<demand_forecasts::DemandIntermittentGenLimitDay1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("OPERATIONAL_DEMAND", "FORECAST", 1_i32) => {
                    #[cfg(feature = "demand_forecasts")]
                    {
                        let d: Vec<demand_forecasts::OperationalDemandForecast1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("GENERIC_CONSTRAINT", "EMSMASTER", 1_i32) => {
                    #[cfg(feature = "generic_constraint")]
                    {
                        let d: Vec<generic_constraint::GenericConstraintEmsmaster1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("GENERIC_CONSTRAINT", "GENCONSETINVOKE", 2_i32) => {
                    #[cfg(feature = "generic_constraint")]
                    {
                        let d: Vec<generic_constraint::GenericConstraintGenconsetinvoke2> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("FORCE_MAJEURE", "MARKET_SUSPEND_REGIME_SUM", 1_i32) => {
                    #[cfg(feature = "force_majeure")]
                    {
                        let d: Vec<force_majeure::ForceMajeureMarketSuspendRegimeSum1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("FORCE_MAJEURE", "MARKET_SUSPEND_REGION_SUM", 1_i32) => {
                    #[cfg(feature = "force_majeure")]
                    {
                        let d: Vec<force_majeure::ForceMajeureMarketSuspendRegionSum1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("FORCE_MAJEURE", "MARKET_SUSPEND_SCHEDULE_TRK", 1_i32) => {
                    #[cfg(feature = "force_majeure")]
                    {
                        let d: Vec<force_majeure::ForceMajeureMarketSuspendScheduleTrk1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("FORCE_MAJEURE", "MARKET_SUSPEND_SCHEDULE", 1_i32) => {
                    #[cfg(feature = "force_majeure")]
                    {
                        let d: Vec<force_majeure::ForceMajeureMarketSuspendSchedule1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("MARKET_NOTICE", "MARKETNOTICETYPE", 1_i32) => {
                    ...
                    {
                        let d: Vec<market_notice::MarketNoticeMarketnoticetype1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("MARKET_NOTICE", "PARTICIPANTNOTICETRK", 1_i32) => {
                    #[cfg(feature = "market_notice")]
                    {
                        let d: Vec<market_notice::MarketNoticeParticipantnoticetrk1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("MARKET_NOTICE", "MARKETNOTICEDATA", 1_i32) => {
                    #[cfg(feature = "market_notice")]
                    {
                        let d: Vec<market_notice::MarketNoticeMarketnoticedata1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("PREDISPATCH", "INTERCONNECTR_SENS", 1_i32) => {
                    #[cfg(feature = "pre_dispatch")]
                    {
                        let d: Vec<pre_dispatch::PredispatchInterconnectrSens1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("PREDISPATCH", "INTERCONNECTOR_SOLN", 3_i32) => {
                    #[cfg(feature = "pre_dispatch")]
                    {
                        let d: Vec<pre_dispatch::PredispatchInterconnectorSoln3> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("PREDISPATCH", "BLOCKED_CONSTRAINTS", 1_i32) => {
                    #[cfg(feature = "pre_dispatch")]
                    {
                        let d: Vec<pre_dispatch::PredispatchBlockedConstraints1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("PREDISPATCH", "SCENARIO_DEMAND_TRK", 1_i32) => {
                    #[cfg(feature = "pre_dispatch")]
                    {
                        let d: Vec<pre_dispatch::PredispatchScenarioDemandTrk1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("PREDISPATCH", "REGIONFCASREQUIREMENT", 2_i32) => {
                    #[cfg(feature = "pre_dispatch")]
                    {
                        let d: Vec<pre_dispatch::PredispatchRegionfcasrequirement2> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("PREDISPATCH", "CONSTRAINT_SOLUTION", 5_i32) => {
                    #[cfg(feature = "pre_dispatch")]
                    {
                        let d: Vec<pre_dispatch::PredispatchConstraintSolution5> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("PREDISPATCH", "PRICESENSITIVITIES", 1_i32) => {
                    #[cfg(feature = "pre_dispatch")]
                    {
                        let d: Vec<pre_dispatch::PredispatchPricesensitivities1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("MARKET_CONFIG", "INTRAREGIONALLOC", 1_i32) => {
                    ...
                    {
                        let d: Vec<market_config::MarketConfigIntraregionalloc1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("MARKET_CONFIG", "INTERCONNECTORCONSTRAINT", 1_i32) => {
                    #[cfg(feature = "market_config")]
                    {
                        let d: Vec<market_config::MarketConfigInterconnectorconstraint1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("MARKET_CONFIG", "INTERCONNECTORALLOC", 1_i32) => {
                    #[cfg(feature = "market_config")]
                    {
                        let d: Vec<market_config::MarketConfigInterconnectoralloc1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("MARKET_CONFIG", "INTERCONNECTOR", 1_i32) => {
                    #[cfg(feature = "market_config")]
                    {
                        let d: Vec<market_config::MarketConfigInterconnector1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("MARKET_CONFIG", "MARKET_PRICE_THRESHOLDS", 1_i32) => {
                    #[cfg(feature = "market_config")]
                    {
                        let d: Vec<market_config::MarketConfigMarketPriceThresholds1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("MARKET_CONFIG", "LOSSFACTORMODEL", 1_i32) => {
                    #[cfg(feature = "market_config")]
                    {
                        let d: Vec<market_config::MarketConfigLossfactormodel1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("MARKET_CONFIG", "TRANSMISSIONLOSSFACTOR", 2_i32) => {
                    #[cfg(feature = "market_config")]
                    {
                        let d: Vec<market_config::MarketConfigTransmissionlossfactor2> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("MARKET_CONFIG", "REGIONSTANDINGDATA", 1_i32) => {
                    #[cfg(feature = "market_config")]
                    {
                        let d: Vec<market_config::MarketConfigRegionstandingdata1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("IRAUCTION", "SRA_PRUDENTIAL_COMP_POSITION", 1_i32) => {
                    #[cfg(feature = "irauction")]
                    {
                        let d: Vec<irauction::IrauctionSraPrudentialCompPosition1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("SETTLEMENT_CONFIG", "RESIDUECONTRACTPAYMENTS", 1_i32) => {
                    #[cfg(feature = "irauction")]
                    {
                        let d: Vec<irauction::SettlementConfigResiduecontractpayments1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("IRAUCTION_CONFIG", "AUCTION_CALENDAR", 2_i32) => {
                    #[cfg(feature = "irauction")]
                    {
                        let d: Vec<irauction::IrauctionConfigAuctionCalendar2> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("IRAUCTION", "SRA_FINANCIAL_AUC_MARDETAIL", 1_i32) => {
                    #[cfg(feature = "irauction")]
                    {
                        let d: Vec<irauction::IrauctionSraFinancialAucMardetail1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("IRAUCTION_CONFIG", "AUCTION_IC_ALLOCATIONS", 2_i32) => {
                    #[cfg(feature = "irauction")]
                    {
                        let d: Vec<irauction::IrauctionConfigAuctionIcAllocations2> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("IRAUCTION", "SRA_FINANCIAL_AUCPAY_SUM", 1_i32) => {
                    #[cfg(feature = "irauction")]
                    {
                        let d: Vec<irauction::IrauctionSraFinancialAucpaySum1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("IRAUCTION_CONFIG", "AUCTION_REVENUE_ESTIMATE", 1_i32) => {
                    #[cfg(feature = "irauction")]
                    {
                        let d: Vec<irauction::IrauctionConfigAuctionRevenueEstimate1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("IRAUCTION", "SRA_FINANCIAL_AUC_MARGIN", 1_i32) => {
                    #[cfg(feature = "irauction")]
                    {
                        let d: Vec<irauction::IrauctionSraFinancialAucMargin1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("IRAUCTION", "SRA_FINANCIAL_AUCPAY_DETAIL", 1_i32) => {
                    #[cfg(feature = "irauction")]
                    {
                        let d: Vec<irauction::IrauctionSraFinancialAucpayDetail1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("IRAUCTION_CONFIG", "AUCTION_REVENUE_TRACK", 1_i32) => {
                    #[cfg(feature = "irauction")]
                    {
                        let d: Vec<irauction::IrauctionConfigAuctionRevenueTrack1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("IRAUCTION", "SRA_PRUDENTIAL_EXPOSURE", 1_i32) => {
                    #[cfg(feature = "irauction")]
                    {
                        let d: Vec<irauction::IrauctionSraPrudentialExposure1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("IRAUCTION", "RESIDUE_CON_ESTIMATES_TRK", 1_i32) => {
                    #[cfg(feature = "irauction")]
                    {
                        let d: Vec<irauction::IrauctionResidueConEstimatesTrk1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("IRAUCTION", "SRA_PRUDENTIAL_CASH_SECURITY", 1_i32) => {
                    #[cfg(feature = "irauction")]
                    {
                        let d: Vec<irauction::IrauctionSraPrudentialCashSecurity1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("IRAUCTION", "SRA_FINANCIAL_AUC_RECEIPTS", 1_i32) => {
                    #[cfg(feature = "irauction")]
                    {
                        let d: Vec<irauction::IrauctionSraFinancialAucReceipts1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("IRAUCTION_CONFIG", "AUCTION_RP_ESTIMATE", 1_i32) => {
                    #[cfg(feature = "irauction")]
                    {
                        let d: Vec<irauction::IrauctionConfigAuctionRpEstimate1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("BILLING_CONFIG", "GST_TRANSACTION_CLASS", 1_i32) => {
                    ...
                    {
                        let d: Vec<billing_config::BillingConfigGstTransactionClass1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("BILLING_CONFIG", "SECDEPOSIT_PROVISION", 1_i32) => {
                    #[cfg(feature = "billing_config")]
                    {
                        let d: Vec<billing_config::BillingConfigSecdepositProvision1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("BILLING_CONFIG", "GST_TRANSACTION_TYPE", 1_i32) => {
                    #[cfg(feature = "billing_config")]
                    {
                        let d: Vec<billing_config::BillingConfigGstTransactionType1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("BILLING_CONFIG", "BILLINGCALENDAR", 2_i32) => {
                    #[cfg(feature = "billing_config")]
                    {
                        let d: Vec<billing_config::BillingConfigBillingcalendar2> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("BILLING_CONFIG", "SECDEPOSIT_INTEREST_RATE", 1_i32) => {
                    #[cfg(feature = "billing_config")]
                    {
                        let d: Vec<billing_config::BillingConfigSecdepositInterestRate1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("DISPATCH", "INTERMITTENT_FORECAST_TRK", 1_i32) => {
                    #[cfg(feature = "dispatch")]
                    {
                        let d: Vec<dispatch::DispatchIntermittentForecastTrk1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("STPASA", "INTERCONNECTORSOLN", 2_i32) => {
                    #[cfg(feature = "stpasa_solution")]
                    {
                        let d: Vec<stpasa_solution::StpasaInterconnectorsoln2> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("STPASA", "CONSTRAINTSOLUTION", 2_i32) => {
                    #[cfg(feature = "stpasa_solution")]
                    {
                        let d: Vec<stpasa_solution::StpasaConstraintsolution2> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("GD_INSTRUCT", "INSTRUCTIONSUBTYPE", 1_i32) => {
                    ...
                    {
                        let d: Vec<gd_instruct::GdInstructInstructionsubtype1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("BILLING", "BILLING_CO2E_PUBLICATION_TRK", 1_i32) => {
                    #[cfg(feature = "billing_run")]
                    {
                        let d: Vec<billing_run::BillingBillingCo2ePublicationTrk1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("BILLING", "DIRECTION_RECONCILIATN", 1_i32) => {
                    #[cfg(feature = "billing_run")]
                    {
                        let d: Vec<billing_run::BillingDirectionReconciliatn1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("BILLING", "BILLING_CO2E_PUBLICATION", 1_i32) => {
                    #[cfg(feature = "billing_run")]
                    {
                        let d: Vec<billing_run::BillingBillingCo2ePublication1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("BILLING", "BILLING_DIRECTION_RECON_OTHER", 1_i32) => {
                    #[cfg(feature = "billing_run")]
                    {
                        let d: Vec<billing_run::BillingBillingDirectionReconOther1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("BILLING", "SECDEPOSIT_APPLICATION", 1_i32) => {
                    #[cfg(feature = "billing_run")]
                    {
                        let d: Vec<billing_run::BillingSecdepositApplication1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("SETTLEMENTS", "RESTARTRECOVERY", 6_i32) => {
                    ...
                    {
                        let d: Vec<settlement_data::SettlementsRestartrecovery6> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("SETTLEMENTS", "SET_FCAS_REGULATION_TRK", 1_i32) => {
                    #[cfg(feature = "settlement_data")]
                    {
                        let d: Vec<settlement_data::SettlementsSetFcasRegulationTrk1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("SETTLEMENTS", "ANCILLARY_SUMMARY", 5_i32) => {
                    #[cfg(feature = "settlement_data")]
                    {
                        let d: Vec<settlement_data::SettlementsAncillarySummary5> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("SETTLEMENTS", "LUNLOADPAYMENT", 5_i32) => {
                    #[cfg(feature = "settlement_data")]
                    {
                        let d: Vec<settlement_data::SettlementsLunloadpayment5> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("SETTLEMENTS", "LUNLOADRECOVERY", 5_i32) => {
                    #[cfg(feature = "settlement_data")]
                    {
                        let d: Vec<settlement_data::SettlementsLunloadrecovery5> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("SETTLEMENTS", "REALLOCATIONS", 5_i32) => {
                    #[cfg(feature = "settlement_data")]
                    {
                        let d: Vec<settlement_data::SettlementsReallocations5> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("SETTLEMENTS", "GENDATAREGION", 5_i32) => {
                    #[cfg(feature = "settlement_data")]
                    {
                        let d: Vec<settlement_data::SettlementsGendataregion5> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("SETTLEMENTS", "LSHEDRECOVERY", 5_i32) => {
                    #[cfg(feature = "settlement_data")]
                    {
                        let d: Vec<settlement_data::SettlementsLshedrecovery5> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("SETTLEMENTS", "VICENERGYFIGURES", 5_i32) => {
                    #[cfg(feature = "settlement_data")]
                    {
                        let d: Vec<settlement_data::SettlementsVicenergyfigures5> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("SETTLEMENTS", "RPOWERPAYMENT", 6_i32) => {
                    #[cfg(feature = "settlement_data")]
                    {
                        let d: Vec<settlement_data::SettlementsRpowerpayment6> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("SETTLEMENTS", "RESTARTPAYMENT", 6_i32) => {
                    #[cfg(feature = "settlement_data")]
                    {
                        let d: Vec<settlement_data::SettlementsRestartpayment6> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("SETTLEMENTS", "VICBOUNDARYENERGY", 5_i32) => {
                    #[cfg(feature = "settlement_data")]
                    {
                        let d: Vec<settlement_data::SettlementsVicboundaryenergy5> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("SETTLEMENTS", "INTERVENTIONRECOVERY", 5_i32) => {
                    #[cfg(feature = "settlement_data")]
                    {
                        let d: Vec<settlement_data::SettlementsInterventionrecovery5> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("SETTLEMENTS", "APC_COMPENSATION", 1_i32) => {
                    #[cfg(feature = "settlement_data")]
                    {
                        let d: Vec<settlement_data::SettlementsApcCompensation1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("SETTLEMENTS", "RPOWERRECOVERY", 5_i32) => {
                    #[cfg(feature = "settlement_data")]
                    {
                        let d: Vec<settlement_data::SettlementsRpowerrecovery5> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("SETTLEMENTS", "INTRAREGIONRESIDUES", 5_i32) => {
                    #[cfg(feature = "settlement_data")]
                    {
                        let d: Vec<settlement_data::SettlementsIntraregionresidues5> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("SETTLEMENTS", "NMAS_RECOVERY_RBF", 1_i32) => {
                    #[cfg(feature = "settlement_data")]
                    {
                        let d: Vec<settlement_data::SettlementsNmasRecoveryRbf1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("SETTLEMENTS", "LULOADRECOVERY", 5_i32) => {
                    #[cfg(feature = "settlement_data")]
                    {
                        let d: Vec<settlement_data::SettlementsLuloadrecovery5> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("SETTLEMENTS", "FCASREGIONRECOVERY", 5_i32) => {
                    #[cfg(feature = "settlement_data")]
                    {
                        let d: Vec<settlement_data::SettlementsFcasregionrecovery5> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("SETTLEMENTS", "VICENERGYFLOW", 5_i32) => {
                    #[cfg(feature = "settlement_data")]
                    {
                        let d: Vec<settlement_data::SettlementsVicenergyflow5> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("SETTLEMENTS", "IRPARTSURPLUS", 6_i32) => {
                    #[cfg(feature = "settlement_data")]
                    {
                        let d: Vec<settlement_data::SettlementsIrpartsurplus6> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("VOLTAGE_INSTRUCTION", "INSTRUCTION", 2_i32) => {
                    ...
                    {
                        let d: Vec<voltage_instructions::VoltageInstructionInstruction2> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("VOLTAGE_INSTRUCTION", "TRACK", 2_i32) => {
                    #[cfg(feature = "voltage_instructions")]
                    {
                        let d: Vec<voltage_instructions::VoltageInstructionTrack2> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("SETTLEMENT_CONFIG", "ANCILLARY_RECOVERY_SPLIT", 1_i32) => {
                    #[cfg(feature = "settlement_config")]
                    {
                        let d: Vec<settlement_config::SettlementConfigAncillaryRecoverySplit1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("SETTLEMENT_CONFIG", "MARKET_FEE_EXCLUSION", 1_i32) => {
                    #[cfg(feature = "settlement_config")]
                    {
                        let d: Vec<settlement_config::SettlementConfigMarketFeeExclusion1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("SETTLEMENT_CONFIG", "MARKET_FEE_CAT_EXCL_TRK", 1_i32) => {
                    #[cfg(feature = "settlement_config")]
                    {
                        let d: Vec<settlement_config::SettlementConfigMarketFeeCatExclTrk1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("SETTLEMENT_CONFIG", "MARKET_FEE_EXCLUSION_TRK", 1_i32) => {
                    #[cfg(feature = "settlement_config")]
                    {
                        let d: Vec<settlement_config::SettlementConfigMarketFeeExclusionTrk1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("SETTLEMENT_CONFIG", "MARKETFEETRK", 1_i32) => {
                    #[cfg(feature = "settlement_config")]
                    {
                        let d: Vec<settlement_config::SettlementConfigMarketfeetrk1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("SETTLEMENT_CONFIG", "MARKETFEE", 1_i32) => {
                    #[cfg(feature = "settlement_config")]
                    {
                        let d: Vec<settlement_config::SettlementConfigMarketfee1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("SETTLEMENT_CONFIG", "SETCFG_PARTICIPANT_MPF", 1_i32) => {
                    #[cfg(feature = "settlement_config")]
                    {
                        let d: Vec<settlement_config::SettlementConfigSetcfgParticipantMpf1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("SETCFG", "REALLOCATIONINTERVAL", 1_i32) => {
                    #[cfg(feature = "settlement_config")]
                    {
                        let d: Vec<settlement_config::SetcfgReallocationinterval1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("SETTLEMENT_CONFIG", "MARKETFEEDATA", 1_i32) => {
                    #[cfg(feature = "settlement_config")]
                    {
                        let d: Vec<settlement_config::SettlementConfigMarketfeedata1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("SETTLEMENT_CONFIG", "SETCFG_PARTICIPANT_MPFTRK", 1_i32) => {
                    #[cfg(feature = "settlement_config")]
                    {
                        let d: Vec<settlement_config::SettlementConfigSetcfgParticipantMpftrk1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("SETTLEMENT_CONFIG", "PARTICIPANT_BANDFEE_ALLOC", 1_i32) => {
                    #[cfg(feature = "settlement_config")]
                    {
                        let d: Vec<settlement_config::SettlementConfigParticipantBandfeeAlloc1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("SETTLEMENT_CONFIG", "MARKET_FEE_CAT_EXCL", 1_i32) => {
                    #[cfg(feature = "settlement_config")]
                    {
                        let d: Vec<settlement_config::SettlementConfigMarketFeeCatExcl1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("PARTICIPANT_REGISTRATION", "STATIONOWNER", 1_i32) => {
                    ...
                    {
                        let d: Vec<participant_registration::ParticipantRegistrationStationowner1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("PARTICIPANT_REGISTRATION", "DUDETAILSUMMARY", 4_i32) => {
                    ...
                    {
                        let d: Vec<
                            participant_registration::ParticipantRegistrationDudetailsummary4,
                        > = self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("PARTICIPANT_REGISTRATION", "DUALLOC", 1_i32) => {
                    #[cfg(feature = "participant_registration")]
                    {
                        let d: Vec<participant_registration::ParticipantRegistrationDualloc1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("PARTICIPANT_REGISTRATION", "GENMETER", 1_i32) => {
                    #[cfg(feature = "participant_registration")]
                    {
                        let d: Vec<participant_registration::ParticipantRegistrationGenmeter1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("PARTICIPANT_REGISTRATION", "MNSP_PARTICIPANT", 1_i32) => {
                    ...
                    {
                        let d: Vec<
                            participant_registration::ParticipantRegistrationMnspParticipant1,
                        > = self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("PARTICIPANT_REGISTRATION", "MNSP_INTERCONNECTOR", 2_i32) => {
                    ...
                    {
                        let d: Vec<
                            participant_registration::ParticipantRegistrationMnspInterconnector2,
                        > = self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("PARTICIPANT_REGISTRATION", "DUDETAIL", 3_i32) => {
                    #[cfg(feature = "participant_registration")]
                    {
                        let d: Vec<participant_registration::ParticipantRegistrationDudetail3> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("PARTICIPANT_REGISTRATION", "DISPATCHABLEUNIT", 1_i32) => {
                    ...
                    {
                        let d: Vec<
                            participant_registration::ParticipantRegistrationDispatchableunit1,
                        > = self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("PARTICIPANT_REGISTRATION", "BIDDUIDDETAILS", 1_i32) => {
                    ...
                    {
                        let d: Vec<
                            participant_registration::ParticipantRegistrationBidduiddetails1,
                        > = self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("PARTICIPANT_REGISTRATION", "BIDDUIDDETAILSTRK", 1_i32) => {
                    ...
                    {
                        let d: Vec<
                            participant_registration::ParticipantRegistrationBidduiddetailstrk1,
                        > = self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("PARTICIPANT_REGISTRATION", "PARTICIPANTCLASS", 1_i32) => {
                    ...
                    {
                        let d: Vec<
                            participant_registration::ParticipantRegistrationParticipantclass1,
                        > = self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("PARTICIPANT_REGISTRATION", "GENUNITS_UNIT", 1_i32) => {
                    #[cfg(feature = "participant_registration")]
                    {
                        let d: Vec<participant_registration::ParticipantRegistrationGenunitsUnit1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("PARTICIPANT_REGISTRATION", "GENUNITS", 2_i32) => {
                    #[cfg(feature = "participant_registration")]
                    {
                        let d: Vec<participant_registration::ParticipantRegistrationGenunits2> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("PARTICIPANT_REGISTRATION", "PARTICIPANT", 1_i32) => {
                    #[cfg(feature = "participant_registration")]
                    {
                        let d: Vec<participant_registration::ParticipantRegistrationParticipant1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("PARTICIPANT_REGISTRATION", "STATIONOWNERTRK", 1_i32) => {
                    ...
                    {
                        let d: Vec<
                            participant_registration::ParticipantRegistrationStationownertrk1,
                        > = self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("PARTICIPANT_REGISTRATION", "PARTICIPANTACCOUNT", 1_i32) => {
                    ...
                    {
                        let d: Vec<
                            participant_registration::ParticipantRegistrationParticipantaccount1,
                        > = self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("PARTICIPANT_REGISTRATION", "STATION", 1_i32) => {
                    #[cfg(feature = "participant_registration")]
                    {
                        let d: Vec<participant_registration::ParticipantRegistrationStation1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("PARTICIPANT_REGISTRATION", "STADUALLOC", 1_i32) => {
                    #[cfg(feature = "participant_registration")]
                    {
                        let d: Vec<participant_registration::ParticipantRegistrationStadualloc1> =
                            self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
                ...
                ("PARTICIPANT_REGISTRATION", "PARTICIPANTCATEGORY", 1_i32) => {
                    ...
                    {
                        let d: Vec<
                            participant_registration::ParticipantRegistrationParticipantcategory1,
                        > = self.get_table()?;
                        let total = d.len();
                        let mut current = 0_usize;
                        for chunk in d.chunks(100_000_usize) {
