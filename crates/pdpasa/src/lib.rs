...
where S: futures_util::AsyncRead + futures_util::AsyncWrite + Unpin + Send,
{
    match (
        ...
        (Some("CASESOLUTION"), version) if version <= 3_i32 => {
            let d: Vec<PdpasaCasesolution3> = mms_file.get_table()?;
            mmsdm_core::sql_server::batched_insert(client, file_key, mms_file.header(), &d, "exec mmsdm_proc.InsertPdpasaCasesolution3 @P1, @P2", chunk_size).await?;
        }
        (Some("CONSTRAINTSOLUTION"), version) if version <= 1_i32 => {
            let d: Vec<PdpasaConstraintsolution1> = mms_file.get_table()?;
            mmsdm_core::sql_server::batched_insert(client, file_key, mms_file.header(), &d, "exec mmsdm_proc.InsertPdpasaConstraintsolution1 @P1, @P2", chunk_size).await?;
        }
        (Some("INTERCONNECTORSOLN"), version) if version <= 1_i32 => {
            let d: Vec<PdpasaInterconnectorsoln1> = mms_file.get_table()?;
            mmsdm_core::sql_server::batched_insert(client, file_key, mms_file.header(), &d, "exec mmsdm_proc.InsertPdpasaInterconnectorsoln1 @P1, @P2", chunk_size).await?;
        }
        (Some("REGIONSOLUTION"), version) if version <= 7_i32 => {
            let d: Vec<PdpasaRegionsolution7> = mms_file.get_table()?;
            mmsdm_core::sql_server::batched_insert(client, file_key, mms_file.header(), &d, "exec mmsdm_proc.InsertPdpasaRegionsolution7 @P1, @P2", chunk_size).await?;
        }
        _ => {
