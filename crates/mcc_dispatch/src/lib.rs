...
where S: futures_util::AsyncRead + futures_util::AsyncWrite + Unpin + Send,
{
    match (
        ...
        (Some("CASESOLUTION"), version) if version <= 1_i32 => {
            ...
            mmsdm_core::sql_server::batched_insert(client, file_key, mms_file.header(), &d, "exec mmsdm_proc.InsertMccCasesolution1 @P1, @P2", chunk_size).await?;
        }
        (Some("CONSTRAINTSOLUTION"), version) if version <= 1_i32 => {
            let d: Vec<MccConstraintsolution1> = mms_file.get_table()?;
            mmsdm_core::sql_server::batched_insert(client, file_key, mms_file.header(), &d, "exec mmsdm_proc.InsertMccConstraintsolution1 @P1, @P2", chunk_size).await?;
        }
        _ => {
