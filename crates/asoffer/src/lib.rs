...
where S: futures_util::AsyncRead + futures_util::AsyncWrite + Unpin + Send,
{
    match (
        ...
        (Some("OFFERAGCDATA"), version) if version <= 1_i32 => {
            ...
            mmsdm_core::sql_server::batched_insert(client, file_key, mms_file.header(), &d, "exec mmsdm_proc.InsertAsofferOfferagcdata1 @P1, @P2", chunk_size).await?;
        }
        (Some("OFFERASTRK"), version) if version <= 1_i32 => {
            let d: Vec<AsofferOfferastrk1> = mms_file.get_table()?;
            mmsdm_core::sql_server::batched_insert(client, file_key, mms_file.header(), &d, "exec mmsdm_proc.InsertAsofferOfferastrk1 @P1, @P2", chunk_size).await?;
        }
        (Some("OFFERLSHEDDATA"), version) if version <= 1_i32 => {
            let d: Vec<AsofferOfferlsheddata1> = mms_file.get_table()?;
            mmsdm_core::sql_server::batched_insert(client, file_key, mms_file.header(), &d, "exec mmsdm_proc.InsertAsofferOfferlsheddata1 @P1, @P2", chunk_size).await?;
        }
        (Some("OFFERRESTARTDATA"), version) if version <= 1_i32 => {
            let d: Vec<AsofferOfferrestartdata1> = mms_file.get_table()?;
            mmsdm_core::sql_server::batched_insert(client, file_key, mms_file.header(), &d, "exec mmsdm_proc.InsertAsofferOfferrestartdata1 @P1, @P2", chunk_size).await?;
        }
        (Some("OFFERRPOWERDATA"), version) if version <= 1_i32 => {
            let d: Vec<AsofferOfferrpowerdata1> = mms_file.get_table()?;
            mmsdm_core::sql_server::batched_insert(client, file_key, mms_file.header(), &d, "exec mmsdm_proc.InsertAsofferOfferrpowerdata1 @P1, @P2", chunk_size).await?;
        }
        _ => {
